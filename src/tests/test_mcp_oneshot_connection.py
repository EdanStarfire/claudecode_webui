...
async def test_http_oauth_bearer_header_injected_and_tools_returned():
    ...
    with (
        patch("src.mcp.oneshot_connection.ClientSession", return_value=sess),
        patch("src.mcp.oneshot_connection.streamablehttp_client", fake_http),
    ):
        result = await connector.test_connect(cfg)
...
async def test_sse_no_oauth_header_when_oauth_disabled():
    ...
    with (
        patch("src.mcp.oneshot_connection.ClientSession", return_value=sess),
        patch("src.mcp.oneshot_connection.sse_client", fake_sse),
    ):
        result = await connector.test_connect(cfg)
...
async def test_issue_1800_cleanup_failure_after_success_returns_failed_not_raises():
    ...
    with (
        patch("src.mcp.oneshot_connection.ClientSession", return_value=sess),
        patch("src.mcp.oneshot_connection.streamablehttp_client", fake_http_raises_on_close),
    ):
        result = await connector.test_connect(cfg)
