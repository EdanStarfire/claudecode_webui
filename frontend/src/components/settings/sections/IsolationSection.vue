...
const isolationFields = computed(() => FIELD_SCHEMAS.isolation.map(f => {
  ...
  if (f.key === 'sandbox_config')       return { ...f, description: 'CLI-native sandbox credential masking. Applies only to sessions with Sandbox Mode enabled — unrelated to Docker-based isolation.' }
