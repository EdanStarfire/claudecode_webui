<template>
  <div class="settings-section">
    ...
    <div v-else-if="isNotApplicable" class="section-na">
      <p>System Prompt fields are not applicable for a <strong>{{ entity.area }}</strong> profile.</p>
    </div>
...
const PROFILE_AREA = 'system_prompt'
const SECTION_KEY  = 'system-prompt'
...
const systemPromptFields = computed(() => FIELD_SCHEMAS.system_prompt.map(f => {
  if (f.key === 'system_prompt') return { ...f, placeholder: 'Optional default system prompt for this template…' }
  return f
}))
...
const { fieldStates } = useEditSectionFieldStates({ isTemplateMode, baseConfig, draft, boundProfile, schemaFields: FIELD_SCHEMAS.system_prompt })
