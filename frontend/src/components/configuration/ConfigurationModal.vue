...
const CONFIG_FIELDS = {
  ...
  allowed_tools: {
    ...
    contexts: ['session', 'template', 'ephemeral', 'update'],
    trackState: true,
    toPayload: (v) => { const l = v.split(',').map(s => s.trim()).filter(Boolean); return l },
  ...
  assigned_secrets: {
    ...
    toPayload: (v) => { const arr = Array.isArray(v) ? v.filter(Boolean) : (v ? String(v).split(',').map(s => s.trim()).filter(Boolean) : []); return arr },
  ...
  docker_proxy_allowlist_domains: {
    ...
    contexts: ['session', 'template', 'ephemeral', 'update'],
    trackState: true,
    fromSource: (s) => { const v = s.docker_proxy_allowlist_domains; if (!v) return []; if (Array.isArray(v)) return v.filter(Boolean); return v.split(',').map(s => s.trim()).filter(Boolean) },
    toPayload: (v) => { const arr = Array.isArray(v) ? v.filter(Boolean) : (v ? String(v).split(',').map(s => s.trim()).filter(Boolean) : []); return arr },
    compare: (form, orig) => JSON.stringify(form || []) !== JSON.stringify(orig || []),
  },
...
function computeTemplateDiff() {
  ...
  const scalarFields = [
    ...
    { field: 'docker_proxy_enabled', label: 'Network Proxy' },
    { field: 'docker_proxy_image', label: 'Proxy Image' },
