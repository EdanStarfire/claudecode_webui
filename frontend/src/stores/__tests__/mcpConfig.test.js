...
describe('mcpConfig store — testConnect (issue #1800)', () => {
  async function getStore() {
    const { useMcpConfigStore } = await import('@/stores/mcpConfig')
    return useMcpConfigStore()
  }
  ...
  it('caches the connected result and clears checkingToolsIds on success', async () => {
    ...
    const promise = store.testConnect('cfg-1')
    expect(store.checkingToolsIds.has('cfg-1')).toBe(true)
    const result = await promise

    expect(apiMock.post).toHaveBeenCalledWith('/api/mcp-configs/cfg-1/test-connect', {})
    expect(result.status).toBe('connected')
    expect(store.toolsByConfigId.get('cfg-1')).toEqual(result)
    expect(store.checkingToolsIds.has('cfg-1')).toBe(false)
  })

  it('tracks two concurrent checks independently (mirrors #1799 regression)', async () => {
    let resolveA
    apiMock.post.mockImplementation((url) => {
      if (url === '/api/mcp-configs/cfg-a/test-connect') {
        return new Promise((resolve) => { resolveA = resolve })
      }
      return Promise.resolve({ status: 'connected', stage: null, tools: [], error: null })
    })
    const store = await getStore()

    const promiseA = store.testConnect('cfg-a')
    expect(store.checkingToolsIds.has('cfg-a')).toBe(true)

    await store.testConnect('cfg-b')
    expect(store.checkingToolsIds.has('cfg-a')).toBe(true)
    expect(store.checkingToolsIds.has('cfg-b')).toBe(false)

    resolveA({ status: 'connected', stage: null, tools: [], error: null })
    await promiseA
    expect(store.checkingToolsIds.has('cfg-a')).toBe(false)
  })

  it('caches a failed fallback (not a throw) when the request errors', async () => {
    apiMock.post.mockRejectedValue({ data: { detail: 'connection refused' }, message: 'boom' })
    const store = await getStore()

    const result = await store.testConnect('cfg-2')

    expect(result).toEqual({ status: 'failed', stage: null, tools: [], error: 'connection refused' })
    expect(store.toolsByConfigId.get('cfg-2')).toEqual(result)
    expect(store.checkingToolsIds.has('cfg-2')).toBe(false)
  })

  it('drops a cached tools result when the config is updated (shares cache with fetchTools)', async () => {
    apiMock.post.mockResolvedValue({ status: 'connected', stage: null, tools: [{ name: 'ping' }], error: null })
    apiMock.put.mockResolvedValue({ id: 'cfg-3', name: 'renamed' })
    const store = await getStore()

    await store.testConnect('cfg-3')
    expect(store.toolsByConfigId.get('cfg-3')).toBeDefined()
  ...
  it('drops a cached tools result when the config is deleted (shares cache with fetchTools)', async () => {
    apiMock.post.mockResolvedValue({ status: 'connected', stage: null, tools: [{ name: 'ping' }], error: null })
    apiMock.delete.mockResolvedValue({ deleted: true })
    const store = await getStore()

    await store.testConnect('cfg-4')
    expect(store.toolsByConfigId.get('cfg-4')).toBeDefined()
