...
from backend.docker_utils import resolve_docker_cli_path
...
async def test_session_config_assigned_secrets_propagation(tmp_path):
    ...
    from backend.session_manager import SessionInfo, SessionManager
...
def test_addon_load_no_session_files_leaves_records_empty():
    ...
    with patch("backend.docker.proxy.addon.Path") as mock_path_cls:
        mock_path = MagicMock()
