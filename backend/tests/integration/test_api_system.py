...
# TestConfig (GET/PUT /api/config) removed here — issue #498: config.py stayed
# Frontend-side with split ownership (Frontend owns networking/backend_connection,
# Backend owns everything else, merged-read on the Frontend route). Backend no longer
# serves /api/config at all, so this coverage doesn't belong in Backend's integration
# suite. It returns once Phase 2 rewrites config.py as split-ownership/merged-read —
# see src/tests/test_config_router.py for the router-level coverage that already
# exists and will pass again once that Phase 2 work lands.
...
class TestSkills:
    ...
    async def test_sync_skills(self, api_integration_env):
        ...
        # Ensure sync is enabled first. Issue #498: /api/config moved Frontend-side
        # (config.py split ownership, Phase 2) — Backend's own integration tests set
        # config directly via AppConfigManager instead of the no-longer-local HTTP route.
        config = await webui.app_config_manager.get_config()
        config.features.skill_sync_enabled = True
        await webui.app_config_manager.save_config(config)
    ...
    async def test_sync_skills_when_disabled(self, api_integration_env):
        ...
        # Disable sync. Issue #498: /api/config moved Frontend-side (config.py split
        # ownership, Phase 2) — set config directly via AppConfigManager instead.
        config = await webui.app_config_manager.get_config()
        config.features.skill_sync_enabled = False
        await webui.app_config_manager.save_config(config)
...
class TestRestartCustomPath:
    ...
    def _patches(self, webui, run_git_command_side_effect, run_side_effect):
        return [
            ...
            patch("backend.routers.system.subprocess.run", side_effect=run_side_effect),
            patch("backend.routers.system.asyncio.create_subprocess_exec",
                  AsyncMock(return_value=_fake_async_proc())),
            patch("backend.routers.system.os.execv"),
