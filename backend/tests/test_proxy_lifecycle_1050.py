...
from backend.config_manager import AppConfig, ProxyConfig
...
async def test_effective_proxy_image_enabled_default(tmp_path):
    ...
    with patch("backend.config_manager.load_config") as mock_load:
...
async def test_session_coordinator_routes_token_to_proxy_only(tmp_path):
    ...
    with patch("backend.docker_utils.resolve_docker_cli_path", side_effect=fake_resolve):
