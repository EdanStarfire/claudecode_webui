...
class TestIssue1972InterruptSuccessRecordIdentity:
    """Issue #1972: the two `interrupt_success` system-message send sites in
    claude_sdk.py go straight to message_callback with no storage call at all (by
    the confirmed Option A decision — no stored counterpart is added). Each must
    still stamp its own freshly-minted `message_id` so it carries a real identity
    for live-dedup purposes, rather than reaching the callback with none at all."""
    ...
    @pytest.fixture
    def session_id(self):
        return "test-session-1972-interrupt-success"

    @pytest.mark.asyncio
    async def test_direct_interrupt_path_stamps_message_id(self, temp_dir, session_id):
        """interrupt_session()'s direct client.interrupt() path (issue #748) —
        the interrupt_success notification bypasses the message queue entirely."""
        received = []
        ...
        sdk.info.state = SessionState.RUNNING
        sdk._sdk_client = AsyncMock()

        result = await sdk.interrupt_session()

        assert result is True
        assert len(received) == 1
        assert received[0]["subtype"] == "interrupt_success"
        assert received[0].get("message_id"), (
            "interrupt_success (direct-interrupt path) must carry a message_id"
        )

    @pytest.mark.asyncio
    async def test_queued_interrupt_request_path_stamps_message_id(self, temp_dir, session_id):
        """The message-queue-driven interrupt_request path inside
        _message_processing_loop — a separate send site from the direct path above."""
        received = []
        sdk = ClaudeSDK(
            session_id=session_id,
            working_directory=temp_dir,
        ...
        sdk.message_callback = cb
        sdk._sdk_options = object()
        sdk._message_queue.put_nowait({"type": "interrupt_request"})
        ...
        class _FakeQuery:
            ...
            async def _gen(self):
                ...
                if False:  # pragma: no cover - never reached, keeps this an async generator
                    yield None
        ...
        with patch("backend.claude_sdk.ClaudeSDKClient", return_value=_FakeClient()):
            await sdk._message_processing_loop()

        assert len(received) == 1
        assert received[0]["subtype"] == "interrupt_success"
        assert received[0].get("message_id"), (
            "interrupt_success (queued interrupt_request path) must carry a message_id"
        )
