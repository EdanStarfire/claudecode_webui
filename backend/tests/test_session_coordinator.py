...
class TestIssue819DockerHistoryMount:
    ...
    async def test_issue_819_history_in_extra_mounts(self, temp_coordinator):
        ...
        with patch("backend.docker_utils.resolve_docker_cli_path", fake_resolve):
            with patch("backend.skill_manager.NEW_GLOBAL_SKILLS_DIR") as mock_skills_dir:
...
class TestIssue1088CredFilePermissions:
    ...
    async def test_issue_1088_creds_path_is_world_readable(self, temp_coordinator):
        ...
        with (
            _patch("backend.credential_vault.set_secret_value"),
            _patch("backend.credential_vault.get_secret_value", return_value="ghp_real"),
        ):
        ...
        with (
            _patch("backend.credential_vault.get_secret_value", return_value="ghp_real"),
            _patch("backend.docker_utils.resolve_docker_cli_path", return_value=("/usr/bin/docker", {})),
            _patch("backend.skill_manager.NEW_GLOBAL_SKILLS_DIR") as mock_skills_dir,
        ):
            mock_skills_dir.exists.return_value = False
            await coordinator.start_session(session_id)
...
class TestIssue1402ListSessionsEffectiveConfig:
    ...
    async def test_list_sessions_handles_resolution_failure(self, temp_coordinator):
        ...
        # template-linked one.
        with patch("backend.session_coordinator.resolve_effective_config", side_effect=RuntimeError("simulated")):
            result = await coordinator.list_sessions()
...
class TestIssue1628CompactBoundaryDistillation:
    ...
    async def test_issue_1628_distillation_disabled_no_task(self, temp_coordinator):
        ...
        with patch.object(
            ...
            'backend.session_coordinator.resolve_effective_config',
    ...
    async def test_issue_1628_distill_compaction_registers_resource(self, temp_coordinator):
        ...
        with patch(
            'backend.history_distiller.distill_session_history',
            new_callable=AsyncMock,
            return_value=True,
