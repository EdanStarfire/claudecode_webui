...
def _install_mitmproxy_stubs():
    ...
    if "backend.docker.proxy.addon" in sys.modules:
        del sys.modules["backend.docker.proxy.addon"]


def _make_addon(records, allowed=None):
    _install_mitmproxy_stubs()
    from backend.docker.proxy import addon as addon_mod
    ...
    a._routing = {"hostname_rewrites": {}, "virtual_key": None, "model_map": {}, "default_model": None}
    a.logger = MagicMock()
    return a
...
class TestMakeChunkFilter:
    ...
    def _filter(self, pairs):
        from backend.docker.proxy.addon import _make_chunk_filter
...
class TestBuildRequestPairs:
    ...
    def _build(self, records, host):
        from backend.docker.proxy.addon import _build_request_pairs
...
class TestBuildResponsePairs:
    ...
    def _build(self, records, host):
        from backend.docker.proxy.addon import _build_response_pairs
...
class TestIssue1425SSEUnbufferedFilter:
    ...
    def test_issue_1425_c1_sse_chunks_forwarded_immediately_no_holdback(self):
        ...
        from backend.docker.proxy.addon import _make_unbuffered_chunk_filter
    ...
    def test_issue_1425_c3_non_sse_response_uses_carry_buffer(self):
        ...
        from backend.docker.proxy.addon import _make_chunk_filter, _make_unbuffered_chunk_filter
