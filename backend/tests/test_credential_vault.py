...
def vault(tmp_path):
    ...
    with (
        patch("backend.credential_vault.set_secret_value") as _set,
        patch("backend.credential_vault.get_secret_value") as _get,
        patch("backend.credential_vault.delete_secret_value") as _del,
    ):
