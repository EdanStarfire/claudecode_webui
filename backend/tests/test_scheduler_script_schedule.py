...
async def test_script_with_stdout_delivers_message(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "CI is red\n", "", False))):
...
async def test_script_with_empty_stdout_discards(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "", "", False))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_script_nonzero_exit_records_error(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(1, "", "something failed", False))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_script_timeout_killed_and_recorded_as_error(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(-1, "", "Script exceeded timeout", True))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_script_in_docker_mode_routes_through_docker_exec(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="container-abc")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "output\n", "", False))) as mock_exec,
        patch("backend.legion.scheduler_service.run_command_on_host") as mock_host,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_script_always_enqueues_immediately_no_pre_wait(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "output\n", "", False))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_failure_count_pauses_after_max_retries_on_error(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(1, "", "fail", False))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_stdout_stderr_capped_at_max_stream_bytes(tmp_session_dir):
    ...
    with patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(1, big, big, False))):
        await svc._fire_script_schedule(schedule, 1000.0)
...
def test_schedule_type_immutable_in_update_request():
    from backend.routers._models import ScheduleUpdateRequest
...
async def test_template_only_docker_config_uses_container(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-template")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "ok\n", "", False))) as mock_exec,
        patch("backend.legion.scheduler_service.run_command_on_host") as mock_host,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_profile_only_docker_config_uses_container(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-profile")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "ok\n", "", False))) as mock_exec,
        patch("backend.legion.scheduler_service.run_command_on_host") as mock_host,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_non_containerized_session_runs_on_host(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "host output\n", "", False))) as mock_host,
        patch("backend.legion.scheduler_service.run_command_in_container") as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_secrets_injected_into_docker_exec_env(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-sec")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "output\n", "", False))) as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_post_rotation_vault_value_reflected_at_fire_time(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-rot")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "done\n", "", False))) as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_no_secret_values_appear_in_logs(tmp_session_dir, caplog):
    ...
    with (
        ...
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-log")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "x\n", "", False))),
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_session_direct_docker_false_overrides_template(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "host\n", "", False))) as mock_host,
        patch("backend.legion.scheduler_service.run_command_in_container") as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_container_exited_surfaces_error_no_host_fallback(tmp_session_dir):
    ...
    with (
        ...
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value=None)),
        patch("backend.legion.scheduler_service.find_session_container_any_state", new=AsyncMock(return_value="ctr-stopped")),
        patch("backend.legion.scheduler_service.run_command_in_container") as mock_exec,
        patch("backend.legion.scheduler_service.run_command_on_host") as mock_host,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_issue_1425_b1_docker_env_uses_placeholder_not_plaintext(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-b1")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "ok\n", "", False))) as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_issue_1425_b2_host_schedule_uses_plaintext(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.run_command_on_host", new=AsyncMock(return_value=(0, "host-out\n", "", False))) as mock_host,
        patch("backend.legion.scheduler_service.run_command_in_container") as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_issue_1425_b3_missing_placeholder_raises_runtime_error(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-b3")),
        patch("backend.legion.scheduler_service.run_command_in_container") as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_issue_1425_b4_extra_env_merges_correctly_with_placeholder(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-b4")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "ok\n", "", False))) as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
...
async def test_issue_1425_b5_no_assigned_secrets_no_placeholder_lookup(tmp_session_dir):
    ...
    with (
        patch("backend.legion.scheduler_service.resolve_effective_config", new=AsyncMock(return_value=effective)),
        patch("backend.legion.scheduler_service.find_session_container", new=AsyncMock(return_value="ctr-b5")),
        patch("backend.legion.scheduler_service.run_command_in_container", new=AsyncMock(return_value=(0, "fine\n", "", False))) as mock_exec,
    ):
        await svc._fire_script_schedule(schedule, 1000.0)
