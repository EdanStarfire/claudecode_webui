...
Walks backend/ and frontend/src/ for `<subject>.<field>` patterns where:
...
Issue #498: the domain code this guard is meant to police (session_config.py,
session_manager.py, template_manager.py, models/minion_template.py) moved to
backend/ — the scan target moved with it.
"""
import re
import tokenize
...
from backend.session_config import CONFIG_FIELDS
...
ALLOWED_PY_FILES = {
    "backend/session_config.py",       # CONFIG_FIELDS definition
    "backend/session_manager.py",      # SessionInfo dataclass + migration helper
    "backend/models/minion_template.py",  # MinionTemplate dataclass
    "backend/template_manager.py",     # MinionTemplate migration helper
    "backend/tests/test_no_flat_config_field_access.py",  # this test
}
